...
| **Workspace/cwd escape handling (the other half of FR-SEC-4)** | Not built. The authority-token path is now refused when it is not a regular file (C-6), but a dispatch whose `cwd` escapes the workspace through a link is neither refused nor resolved safely | FR-SEC-4 is `partial`: the token-path half is measured, this half is not, and no test attempts it |
