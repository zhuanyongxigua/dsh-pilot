...
| FR-SESS-3 | P0 | measured | `persistence/crash::a restart preserves task, session and event identity, and never invents a new session` (asserts exactly one `session.create` reached the Host across the restart); `live/live::a SIGKILL during a live turn leaves durable history and an honest, unresolved outcome`; `persistence/session-create-recovery::FR-SESS-3 crash: a create acknowledged before the session row was written is recovered, and reattach names the preallocated Host session` (the acknowledgement is the source: the recovery sends NO second `session.create`, and the recovered row accepts work the Host applies) | Re-attach does not create a second session. |
...
| FR-STATE-2 | P0 | measured | `fake-host/contract::dropped ack after send becomes uncertain, is never retried, and never duplicates the send`; `persistence/crash::SIGKILL during dispatch…`; `mcp-e2e/stdio::uncertain outcomes are reported as errors so a caller cannot mistake them for success`; `live/live::a SIGKILL during a live turn…`; mutations `durable-intent-after-send` and `ack-loss-reported-as-success` (production, both caught); `persistence/session-create-recovery::FR-STATE-2 a retry of an UNCERTAIN session.create is re-marked dispatching before the send, and its confirmed answer is acknowledged` (the durable row is observed as `dispatching` while the retry's response is held, so the answer can be acknowledged instead of raising `ILLEGAL_TRANSITION`) | Repeating the same logical call returns `previously-sent-outcome-unknown` and never reaches the wire again. Scope: the evidence is example-based across four layers; the seeded crash-interleaving model from the original plan was **not** built, so "exactly once under every interleaving" is not claimed. |
...
| FR-OWN-1 | P0 | measured | `unit/owner-lock::the lock database is held in exclusive locking mode, not merely remembered`; `unit/owner-lock::a second holder is refused, and an uncooperative connection cannot take the file`; `unit/core::exclusive ownership: a second handle is refused, and the holder keeps it while stopped`; `unit/core::the daemon refuses to start when the state directory is already owned`; `persistence/crash::two daemons cannot own one state directory, and a clean release lets the next one in`; mutation `second-owner-not-refused` (production); `unit/owner-lock::FR-OWN-1 the owner lock is taken before the state store is opened, so a refused contender cannot touch the schema` (the contender reports exit `4` `OWNER_HELD` with an unopenable state file, where the control with no owner reports exit `5` `STORAGE_CORRUPT` — the two exits are what prove the order) | Kernel-enforced exclusion: the second holder is refused, an *independent* process that knows nothing about `OwnerLock` cannot take a write lock, and a second daemon exits `4` with `OWNER_HELD`. |
...
## 13. Changes from the review-1108 round (five P1 findings at `d090bac`), with the test that holds each one

Five findings, each a defect the reviewer named on the merge-ready revision, each with the case that fails
without its fix and the negative control that was actually RUN (a control that is not run is a claim, not
evidence). Every control below was executed by reverting `src/` to `d090bac`, rebuilding, and running the
named suite: the fix is not asserted to be load-bearing, it was measured to be.

| Defect at `d090bac` | Test that holds the fix | Negative control that was run |
| --- | --- | --- |
| `markDispatching` accepted only `pending` and `#dispatch` marked `dispatching` only from `pending`, so an authorised retry of an `uncertain` `session.create` sent while the row still said "outcome unknown" — and then `markAcknowledged`, which requires `dispatching`/`sent`, raised `ILLEGAL_TRANSITION`. The retry killed the daemon, and the one response that resolved the uncertainty could never be recorded. | `persistence/session-create-recovery::FR-STATE-2 a retry of an UNCERTAIN session.create is re-marked dispatching before the send, and its confirmed answer is acknowledged` (the row is read as `dispatching` **while the response is held** by the fixture barrier) | reverting the transition: the case fails, and on the unfixed build the retry kills the daemon with `ILLEGAL_TRANSITION` — the runner reported that as this case's failure once the pending request's rejection was caught instead of escaping |
| A `succeeded` `session.create` whose session row was never written (the process died between the durable acknowledgement and `#recordSessionFromCreate`) was answered `ok` with **no session**. The session existed on the Host under the preallocated id and was unreachable: no prompt, cancel or state query could name it. | `persistence/session-create-recovery::FR-SESS-3 crash: a create acknowledged before the session row was written is recovered, and reattach names the preallocated Host session` (the Host's request log shows the recovery sends **no** second `session.create`, and the Host applies a prompt to the recovered row) | reverting: fails with `{"session":null,…,"operation":{"state":"succeeded"}}` and `"result":{"status":"ok"}` — the exact reply the finding describes |
| A non-200 `/api/respond` **after the answer was sent** was classified as a definite refusal, telling a caller the answer never arrived while the Host had taken it. | `fake-host/respond-bounds::FR-APPR-4 a non-200 status after the answer was sent is an unproven delivery, not a refusal` (the fixture's receipt log is the control that the Host took exactly one answer first) | reverting: `receipt: "refused"` (`the host did not accept the answer: unknown-refusal`) for an answer the Host's own log shows it took |
| The reconnect history refetch read **one** page and merely counted `hasMore`, so a gap wider than one page left the older missed events unfetched while the cursor went on describing the stored rows as a complete run. | `fake-host/delivery-integrity::FR-EV-4 a gap wider than one history page is reconciled by following beforeSeq, so the sweep is never silently partial` (250 history-only events against a 200-message page; the OLDEST is asserted present) | reverting: the case times out waiting for the oldest missed event, and `eventStats.historyTruncations` never counts the truncated pages |
| `new Store()` ran in the `Daemon` constructor **before** `start()` took the owner lock, so a contender that was about to be refused had already created the directory, run the DDL and applied migrations to a schema it does not own. | `unit/owner-lock::FR-OWN-1 the owner lock is taken before the state store is opened, so a refused contender cannot touch the schema` (exit `4` `OWNER_HELD` is the observable, with the state file's bytes compared before and after) | reverting: the contender exits `5` with `STORAGE_CORRUPT: opening state: file is not a database`, which is the store having been opened first. The in-test control — the same unopenable state file with **no** owner — reports `5` as well, which is what makes exit `4` evidence of the ORDER rather than of the file |

Three things this round also corrected, none of them cosmetic:

- **A detail named `status` overwrote a `Result`'s own `status`.** The first version of the `/api/respond`
  fix passed `{ status: settled.status }` as an `uncertain(...)` detail; `Result` merges details onto
  itself, so the result became `status: 500`, `result.uncertain` read `false`, and the daemon reported a
  definite refusal — the very misreading the fix existed to remove. The case caught it (`got "refused"`).
  The detail is now `httpStatus`, and the shaper assigns its own classification **last** so no future
  detail can rewrite it. `uncertain`/`refused`/`ok` are the properties every branch tests; they win.
- **A negative control was resting on the defect it was supposed to be independent of.**
  `session-create-id-not-reused-on-retry` anchored on the `hostSessionId` fallback, and it was caught only
  because ignoring that fallback made a reattach report `ok` with `session: null` — the P1 defect this
  round fixed. With the recovery path in place, ignoring the fallback no longer changes any outcome, so the
  control survived a full sweep (`28/30`). The mutation now anchors on the line that decides what a retry
  **sends** (the stored request), and the fake-host case grew two wire-level assertions — both attempts
  must carry the same preallocated id, and the recorded id must be the id the Host was asked for — which
  are the controls for that mutation and for `fake-host-ignores-preallocated-session-id` respectively.
  Both were run and both fail the case: `a retry must reuse the preallocated Host session id…` and
  `the recorded id must be the id the Host was asked for…`. A control that a fix quietly neutralises is a
  control that has stopped testing anything, and the sweep is what said so.
- **Taking the lock first must not break a state directory that does not exist yet.** Locking needs the
  directory, and `Store` used to create it, so the constructor now creates the DIRECTORY before acquiring
  the lock (creating a directory is not opening a store, running DDL or migrating). The ordering claim is
  about the schema, and it stays true.
