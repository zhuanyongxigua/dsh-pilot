...
export default {
  ...
  'FR-APPR-3: a crash while the answer is in flight is reported as unproven, and the answer is never sent twice': async () => {
    ...
    try {
      ...
      try {
        ...
        replay = /** @type {{code?: string, details?: {deliveredToHost?: string|null, hint?: string, deliveryState?: string|null}}} */ (error);
      }
      ...
      // THE OTHER HALF OF THE FINDING. The hint used to be generated by the mere EXISTENCE of a
      // delivery row (`previousDelivery ? 'already delivered' : 'never delivered'`), so an answer whose
      // outcome nobody observed was described to the operator as delivered — turning a possible
      // duplicate approval into a reassuring no-op. It is now derived from the closed receipt set, and
      // `dispatching` is one of the states that must never borrow the delivered wording.
      assert.ok(!/already delivered/i.test(String(replay?.details?.hint ?? '')),
        `the hint for an unproven delivery must not say the answer was delivered: ${String(replay?.details?.hint)}`);
      assert.match(String(replay?.details?.hint ?? ''), /MAY have been applied/,
        `the hint must say the outcome is unproven, got: ${String(replay?.details?.hint)}`);
      assert.equal(replay?.details?.deliveryState, 'dispatching',
        'the detail must name the delivery state, so a caller can branch on the closed set rather than on null-ness');
  ...
  'FR-APPR-3: an unreachable host records a provable refusal, which is a different state from dispatching': async () => {
    ...
    try {
      ...
      // A refused delivery is the case where the old wording was actively harmful: telling an operator
      // "already delivered, re-deciding is a no-op" would suppress the one action that fixes it.
      let replay = null;
      try {
        await r.ipc.request({
          op: 'interaction.decide', taskId: r.taskId, interactionId,
      ...
      } catch (error) { replay = /** @type {{code?: string, details?: {hint?: string, deliveryState?: string|null}}} */ (error); }
      assert.equal(replay?.code, 'APPROVAL_REPLAYED', `expected a typed replay refusal, got ${JSON.stringify(replay)}`);
      assert.equal(replay?.details?.deliveryState, 'refused',
        'the detail must name the refusal, not merely that a row exists');
      assert.ok(!/already delivered/i.test(String(replay?.details?.hint ?? '')),
        `a refused delivery must not be described as delivered: ${String(replay?.details?.hint)}`);
      assert.match(String(replay?.details?.hint ?? ''), /never reached the Host/,
        `the hint must say the answer never reached the Host, got: ${String(replay?.details?.hint)}`);
